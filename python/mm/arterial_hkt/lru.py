...
Taken from https://github.com/stucchio/Python-LRU-cache
'''
...
def lru_cache_function(max_size=1024, expiration=15 * 60):
    ...
    """
    def wrapper(func):
        c = LRUCachedFunction(func, LRUCacheDict(max_size, expiration))
        @wraps(func)
        def fun(*args,**kwargs):
...
class LRUCachedFunction(object):
    ...
    """
    def __init__(self, function, cache=None,obj=None):
        if cache:
            self.cache = cache
        else:
    ...
    def __call__(self, *args, **kwargs):
        ...
        try:
          ...
          if self.obj:
            value = self.function(self.obj,*args, **kwargs)
          else:
